...
class ImapLibrary(object):
    ...
    def _get_emails(self, email):
        if email is None:
            r, item = self.imap.search(None, 'UNSEEN')
        else:
            r, item = self.imap.search(None, 'UNSEEN', 'FROM', email)

        return   item[0].split()

    def get_email_body_export_to_html(self, mailNumber, savePath, fileName):
        """
        Returns a export file full path

        `mailNumber` is the index number of the mail to open
        'savePath' is the file localtion of folder
        ...
        fileFullPath = os.path.join(savePath, fileName+".html")
    ...
    def get_phish_link(self):
        """
        Returns the url for lastest email # EZPH Project
        """
        ...
        return re.findall(r'href=[\'"]?([^\'" >]+)', content)[0]

    def get_trace_img(self):
        """
        Returns the trace img for lastest email # EZPH Project
        """
        criteria = self._criteria()
        status, data = self._imap.select()
        if status != 'OK':
        ...
        body = ''.join(body.replace('\r','').split('\n')[4:-1])
        content = base64.standard_b64decode(body).decode('utf-8')
        return re.findall(r'src=[\'"]?([^\'" >]+)', content)[-1]
