...
class Query(object):
    ...
    def call_procedure(self, name, *parameters):
        ...
        @param parameters: the num of parameters should >= 0

        Examples:
        | Call Procedure | ProcedureName1 | First param |
        | Call Procedure | ProcedureName2 | First param | Second param |
        '''
        cur = None
        try:
            cur = self._dbconnection.cursor()
    ...
    def __callproc_sql(self, cur, procname, *parameters):
        ...
        listOfParams = list(parameters)
        paramsStr = ''

        if len(listOfParams[0]) == 0: # no parameter
            return cur.execute("{call %s}" % (procname))
        else:
            for index, param in enumerate(listOfParams[0]):
                if index == 0: #first element
                    paramsStr += '\''+ param + '\''
                else: 
                    paramsStr += ',\''+ param + '\''
            return cur.execute("{call %s(%s)}" % (procname, paramsStr))
